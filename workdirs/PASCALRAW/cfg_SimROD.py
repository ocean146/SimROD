...
class Exp(YoloXBaseExp):
    def __init__(self):
        ...
        self.data_dir = '/data4022/xiehaiyang/cvpr25/SimROD/SimROD_vs_DIAP/data/PASCALRAW/'
        self.train_ann = 'pascalraw_train.json'
        self.val_ann = 'pascalraw_val.json'
        self.test_ann = 'pascalraw_val.json'
        self.train_ims  = 'original/demosaic'
        self.val_ims = 'original/demosaic'
        self.test_ims = 'original/demosaic'
        # ---------------- transform config ---------------- #
    ...
    def get_model(self, sublinear=False):
        ...
        if "model" not in self.__dict__:
            ...
            if hasattr(self,"tmm"):
                self.model = YOLOX(backbone, head, nf=16, gamma_range=self.gamma_range,tmm=self.tmm)
            else:
                self.model = YOLOX(backbone, head, nf=16, gamma_range=self.gamma_range,tmm="both")

        self.model.apply(init_yolo)
        self.model.head.initialize_biases(1e-2)

        import warnings
        warnings.warn("load partial weights from coco pretrain weights")
        origin = torch.load("./weights/yolox_nano.pth")['model']
        old_keys = [key for key in origin.keys() if 'backbone.backbone.stem.conv.' in key or 'head.cls_preds' in key]
        for i in range(len(old_keys)):
            old_key = old_keys[i]
            origin.pop(old_key)
        
        model_state_dict = self.model.state_dict()
        model_state_dict.update(origin)
