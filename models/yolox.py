...
from .rggb_module import SimpeRGGBGammaEnhancedv6_GreenSEv4_duplicatedG
...
class YOLOX(nn.Module):
    ...
    """
    def __init__(self, backbone=None, head=None, nf=16, gamma_range=[1.,4.],tmm="both",rod=False):
        super().__init__()
        # 2023-04-19 Modified by Huawei, definition of adaptive adjustment module
        # 2025-03-11 Modified by Haiyang Xie
        if 'diap' in tmm.split(','):
            self.TMM = Adaptive_Module(in_ch=3, nf=nf, gamma_range=gamma_range)
        elif 'SimROD' in tmm.split(','):
            self.TMM = SimpeRGGBGammaEnhancedv6_GreenSEv4_duplicatedG(gamma_range=gamma_range)
        else:
