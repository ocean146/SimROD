...
class Trainer:
    def __init__(self, exp, args):
        ...
        folder_name = F"{self.exp.tmm}"
        self.file_name = os.path.join(exp.output_dir, folder_name)
    ...
    def before_train(self):
        ...
        if self.is_distributed:
            ...
            # model = DDP(model, device_ids=[self.local_rank], broadcast_buffers=False, find_unused_parameters=True)      # modified by xhy to debug
    ...
    def before_epoch(self):
        ...
        begin_train_all_param_epoch = getattr(self.exp,"train_all_params_epoch",self.max_epoch+3)
        if self.epoch+1 >= begin_train_all_param_epoch:
            for name, param in self.model.named_parameters():
                param.requires_grad = True
            logger.info("---> begin train all params")
            ...
            if self.rank == 0:
                for k,v in loss_meter.items():
                    self.tblogger.add_scalar(
                        F"loss/{k}",
                        v.latest.item() if isinstance(v.latest, torch.Tensor) else float(v.latest),
                        int(self.epoch*self.max_iter + self.iter)
                    )
                if self.is_distributed:
                    model_print = self.model.module
                else:
                    model_print = self.model
                for k,v in model_print.named_modules():
                    if hasattr(v,'p') and isinstance(v.p, torch.nn.Parameter):
                        self.tblogger.add_scalar(
                            "loss/p",
                            v.p.item(),
                            int(self.epoch*self.max_iter + self.iter)
