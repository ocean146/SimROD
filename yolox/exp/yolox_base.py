        ...
        if "optimizer" not in self.__dict__:
            ...
            for k, v in self.model.named_modules():
                ...
                if isinstance(v, nn.BatchNorm2d) or "bn" in k:
                    if v.weight is not None:  # ocean_modifed
                        pg0.append(v.weight)  # no decay
                elif hasattr(v, "weight") and isinstance(v.weight, nn.Parameter):
                    pg1.append(v.weight)  # apply decay
                if hasattr(v,"p") and isinstance(v.p, nn.Parameter):  # ocean_modifed
                    pg3.append(v.p)  # ocean_modifed
            
            for k,v in self.model.named_parameters():
                if k.startswith("TMM.my_"):
                    pg3.append(v)
            ...
            if len(pg3):
                optimizer.add_param_group({"params": pg3, "weight_decay": self.weight_decay}) # ocean_modifed
